...
export class PatientController {
  ...
  async bookByHn(
    ...
    let patient = await this.patientRepository.findOne({where: {hn: body.hn}});
    const bookTemplate = await this.bookingRepository.findOne({where: {code: body.code}});

    if (bookTemplate == null) {
      // error book not exist
    }
    if (patient == null) {
      // find patient in mssql
      // insert in our mongodb
      // find inserted in mongodb by use HN
      // set patient to inserted
    }
    patient?.bookings?.push(bookTemplate);
    // await this.patientRepository.updateById(patient.id, patient);
  }
}
